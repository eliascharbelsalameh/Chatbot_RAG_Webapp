# main.py — LEGACY (USEK Amanda assistant)
#
# This is the original Streamlit app from the USEK semester project. It is kept
# for reference only. The maintained UI is `../app.py`. Known issues with this
# file (intentionally not fixed):
#   - depends on Deepgram (paid API)
#   - downloads a BART summarizer at startup
#   - depends on st_audiorec, pyttsx3, pyperclip
#   - uses sibling legacy modules (faiss_utils, audio_*, session_utils, data_processing)
#
# Do not run from this folder; `streamlit run app.py` from the repo root instead.
